// backend/scripts/migrate-formas-pago-completo.js
...
// TODAS LAS FORMAS DE PAGO PARA CADA CADENA - VERSIÓN COMPACTA (solo cadena 10 para probar)
const FORMAS_PAGO_CADENA_10 = [
...
async function migrateFormasPago() {
    try {
        ...
        console.log('🧹 Colección formaspagos limpiada');

        let totalInsertados = 0;

        // Insertar para cadena 10 (KFC)
        console.log(`\n📦 Insertando formas de pago para cadena 10...`);

        for (let i = 0; i < FORMAS_PAGO_CADENA_10.length; i++) {
            const fp = FORMAS_PAGO_CADENA_10[i];
            try {
                ...
                totalInsertados++;
                if (i < 10) {
                    console.log(`  ✅ Insertado: ${fp.nombre}`);
                } else if (i === 10) {
                    console.log(`  ... y ${FORMAS_PAGO_CADENA_10.length - 10} más`);
                }
            } catch (err) {
                console.log(`  ⚠️ Error insertando ${fp.nombre}:`, err.message);
            }
        }

        // Verificar que los datos se guardaron
        const totalEnDB = await FormaPago.countDocuments();
        console.log(`\n📊 Total en base de datos: ${totalEnDB}`);

        // Verificar específicamente cadena 10
        const cadena10Count = await FormaPago.countDocuments({ cadenaId: "10", activo: true });
        console.log(`📊 Formas de pago para cadena 10: ${cadena10Count}`);

        // Mostrar los primeros 5 de cadena 10
        const muestraCadena10 = await FormaPago.find({ cadenaId: "10", activo: true }).limit(5);
        console.log('📋 Muestra cadena 10:');
        muestraCadena10.forEach(f => console.log(`   - ${f.nombre}`));
